...
if file is not None:
    ...
    type_of_finding = st.sidebar.selectbox("Options",('-','EDA', 'Sampling Techniques'))   
    ...
    if type_of_finding == '-':
       ...
       if agree:
           ...
           def what_is_best_t(df):
               min_absolute_errors = df.groupby(['Confidence Interval', 'Sampling Technique'])['Absolute Error'].min().reset_index()
           ...
           with st.expander("Show The Conclusion"):
               ...
               for confidence_interval in best_techniques['Confidence Interval'].unique():
                   ...
                   st.write(f"{confidence_interval} Confidence interval ---> The Best Technique is {technique}.")
